...
describe("Token", function () {
  ...
  describe("Token Deployment", function () {
    it("should deploy to an address", async () => {
      ...
      expect(await token.IMAGE_URI()).to.equal(
  ...
  describe("Token Core Contract", function () {
    ...
    it("should burn correctly", async () => {
      ...
      await expect(token.burn(account1addy, 0, 1)).to.be.revertedWith(
    ...
    it("should return the uri for a token ID", async () => {
      ...
      expect(await token.uri(1)).to.equal(
        "https://ipfs.io/ipfs/QmTdMVgk11h5hFjp7vbnb8h8NFm1o9rQZjoBSoiDpYSDwi/1"
      );
      expect(await token.uri(100)).to.equal(
