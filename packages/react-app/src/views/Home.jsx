...
function Home({ address, readContracts, writeContracts, tx, connected }) {
  ...
  let currentApproval = useContractReader(readContracts, "Token", "isApprovedForAll(address,address)", [
    ethers.utils.getAddress(address ? address : anyAddress),
    ethers.utils.getAddress(theForgingAddress ? theForgingAddress : anyAddress),
  ]);
  ...
  const getApproval = async () => {
    if (!currentApproval) {
      ...
      let result = tx(
        writeContracts.Token.setApprovalForAll(
          ethers.utils.getAddress(theForgingAddress ? theForgingAddress : anyAddress),
          1,
        ),
        update => {
      ...
      <Col span={24}>
        {connected ? (
          <>
                ...
                <div align="right">
                  {currentApproval ? (
                    ...
                    <Button
                      block
                      size="medium"
                      style={{ marginLeft: "10px", borderColor: "rgba(0,0,0,0)", color: "rgba(255,255,255,0)" }}
                      loading={loadings[100]}
                      ghost
                      disabled
                      icon={<CloseCircleOutlined />}
            ...
            <Title level={4} style={{ color: "white", fontFamily: "futura" }}>
              Raw Materials
            </Title>
                                ...
                                let result = tx(writeContracts.Forging.mint(BigNumber.from("0")), update => {
                                  if (update && (update.status === "confirmed" || update.status === 1)) {
                                    ...
                                    (update.status === "failed" ||
                                      (update.code === -32603 && update.data.message.includes("Cooldown")))
                                  ) {
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                if (checkedList.length > 0) {
                                  ...
                                  let convertedNumbers = checkedList.map(item => {
                                    return BigNumber.from(item);
                                  });
                                  let result = tx(
                                    writeContracts.Forging.trade(convertedNumbers, BigNumber.from(target)),
                                    update => {
                                      if (currentApproval) {
                                        if (update && (update.status === "confirmed" || update.status === 1)) {
                    ...
                    {anyTokens !== undefined && anyTokens ? (
                      <>
                        {numUnique !== undefined && numUnique > 1 ? (
                          <Title level={5} style={{ color: "white", fontFamily: "futura" }}>
                            Select two or more elements to forge weaponry
                          </Title>
                        ) : (
                          <Title level={5} style={{ color: "#CFCFC4", fontFamily: "futura" }}>
                            You need two or more elements
                            <br />
                            to forge weaponry
                          </Title>
                        )}
                      ...
                      <>
                        {anyTokens !== undefined && anyTokens === false ? (
                          <>
                            <Title level={5} style={{ color: "#FF5733", fontFamily: "futura" }}>
                              You are out of raw materials.
                              <br />
                              Mint more above.
                            </Title>
                          </>
                            ...
                            {myTokens && myTokens[0] !== undefined && myTokens[0] > 0 ? (
                              <>
                                <div align="center">
                                  ...
                                  >
                                    <Checkbox value="0" style={{ color: "white" }}>
                                      Iron
                                    </Checkbox>
                                  </Card>
                                </div>
                        ...
                        <Col span={8}>
                          <div align="center" style={{ fontFamily: "futura", color: "white" }}>
                            {myTokens && myTokens[1] !== undefined && myTokens[1] > 0 ? (
                              ...
                              >
                                <Checkbox value="1" style={{ color: "white" }}>
                                  Carbon
                                </Checkbox>
                              </Card>
                            ) : null}
                            ...
                            {myTokens && myTokens[2] !== undefined && myTokens[2] > 0 ? (
                              <>
                                <div align="center">
                                  ...
                                  >
                                    <Checkbox value="2" style={{ color: "white" }}>
                                      Wood
                                    </Checkbox>
                                  </Card>
                                </div>
                    ...
                    <>
                      {checkedList.length >= 2 ? (
                        <>
                          <Col flex="100px">
                            <Button
                              ...
                              size="large"
                              shape="round"
                              style={{ margin: 2, fontFamily: "futura" }}
                              icon={<FireTwoTone twoToneColor="#ff9a00" />}
                              loading={loadings[6]}
                              onClick={async () => {
                                enterLoading(6);
                                await getApproval();
                        ...
                        <>
                          <Col flex="100px">
                            {anyTokens !== undefined && anyTokens ? (
                              <Button block size="large" shape="round" style={{ fontFamily: "futura" }} disabled>
                                choose elements
                              </Button>
                            ) : null}
                          </Col>
